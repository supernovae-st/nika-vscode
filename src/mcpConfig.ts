...
export async function ensureCursorRules(log: LogFn, providers?: RulesIntel): Promise<void> {
  ...
  const content = [
    ...
    '',
    '## Key Rules',
    '- Interpolation · 6 namespaces. The FOUR value authorities: ${{ inputs.x }} (typed, caller-supplied) · ${{ config.x }} (deployment config, may be logged) · ${{ const.x }} (fixed value) · ${{ secrets.x }} (vault-backed, masked). Plus the two runtime ones: ${{ with.alias }} · ${{ tasks.id.output }}',
    ...
    '- inputs:/config: entries are TYPED declarations (`type:` required). Types are the closed grammar: null · bool · integer · number · string · bytes · uri · path · duration · timestamp, plus constructors { array: T } · { map: T } · { object: {…} } · { union: […] } · { enum: […] }. `boolean` and bare `array`/`object` are NOT types',
    '- output: named jq bindings · outputs: = the workflow return value',
    '- when: CEL conditional, LOCAL reads only (the value authorities · with · item/index) · retry: { max_attempts, backoff } · on_error: recover:',
