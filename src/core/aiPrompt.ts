...
export function buildAuthoringPrompt(workflowPath?: string): string {
  ...
  return [
    ...
    '   (plain words stay incomplete: `nika compile "<describe the job>" ' + target + '`).',
    '2. GROUND — read the embedded contract, never invent fields:',
    '   `nika spec` (language surface) · `nika schema` (JSON Schema) ·',
    '   bare `nika try` (the catalog) · `nika compile <slug> <file>` writes one whole.',
    ...
    'read the findings; review proposed fixes before executing any command.',
    '',
    'PROVE runs, never assert them: the engine writes a journal to',
    '`.nika/traces/*.ndjson` — quote it. `nika trace outputs <trace>` (per-task',
    'table) · `nika trace peek <trace> <task> --raw` (one exact output — pipe',
    'to jq) · `nika trace verify <trace> --json` reports attained proof tiers',
    'and gaps; hash consistency alone is not a signature or external anchor ·',
