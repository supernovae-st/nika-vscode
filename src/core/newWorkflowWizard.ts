...
// (positional `nika compile <template> dest` writes the file, its models live inside), so the
  ...
  if (templates.length > 0) {
    ...
    for (const slug of templates) {
      rows.push({
        ...
        detail: 'nika compile <template> dest.nika — the engine writes the whole file',
