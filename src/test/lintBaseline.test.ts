...
describe('grandfatherMask', () => {
  const baseline = parseBaseline(JSON.stringify({
    captured: '2026-07-05',
    counts: { 'wf/a.nika::NIKA-VAR-001': 2, 'wf/a.nika::NIKA-DAG-003': 1 },
  }));
  ...
  it('a different file shares no budget', () => {
    expect(grandfatherMask('wf/b.nika', ['NIKA-VAR-001'], baseline)).toEqual([false]);
  });

  it('no baseline = everything loud (opt-in ratchet)', () => {
    expect(grandfatherMask('wf/a.nika', ['NIKA-VAR-001'], undefined)).toEqual([false]);
  });
});
...
describe('captureBaseline → parseBaseline round-trip', () => {
  it('folds per-file codes into sorted counts and survives the round-trip', () => {
    const captured = captureBaseline(
      new Map([
        ['wf/a.nika', ['NIKA-VAR-001', 'NIKA-VAR-001']],
        ['wf/b.nika', ['NIKA-SEC-001']],
      ]),
      '2026-07-05',
    );
    expect(captured.counts['wf/a.nika::NIKA-VAR-001']).toBe(2);
    ...
    const after = captureBaseline(new Map([['wf/a.nika', ['NIKA-VAR-001']]]), '2026-07-06');
    expect(after.counts['wf/a.nika::NIKA-VAR-001']).toBe(1);
    expect(after.counts['wf/b.nika::NIKA-SEC-001']).toBeUndefined();
  });
